...
- 可选择约每周自动更新当前规则使用的字幕目录；关闭后不再按时间定期更新，但仍可手动强制刷新
...
影片编号和规范化节目名称目录保存在 `chrome.storage.local`，并按地区、Netflix 资料、Netflix 界面语言和字幕语言分开：首页、电影、节目、分类、片单、最新和搜索页会共用相应缓存。关闭“每周自动更新字幕资料”时，仍然有效且保留在本机的缓存不会仅因经过一段时间而失效。手动点击“清除全部缓存并重新读取”、切换地区、资料、界面语言或所选字幕语言，以及缓存缺失、无效、迁移、被扩展逐出或被浏览器清理时，都可能重新读取目录。
...
Netflix 普通浏览页不会把每张卡片的字幕清单直接写入页面。扩展会使用当前 Netflix 页面已有的登录状态，向 `www.netflix.com` 读取官方“按照语言浏览”目录中的影片编号，再与页面卡片的 Netflix 影片编号比对。请求不会发送到第三方服务器；扩展也不会保存 Cookie、登录令牌或完整 Netflix 响应。`chrome.storage.local` 仅保存精简的影片编号、规范化节目名称索引，以及用于区分和验证缓存所需的元数据。
...
Netflix 官方说明，字幕是否可用可能受到所在地、资料语言偏好、具体影片和设备影响。缓存会按国家或地区代码，以及根据当前资料与 Netflix 界面区域设置生成的不可读散列分开；扩展不会保存原始资料 ID。这个扩展不会改变地区、绕过授权或下载 Netflix 视频，只会读取账号当前可用的官方字幕目录并显示本地字幕覆盖层。
