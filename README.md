...
一个不依赖自建服务器的 Chrome Manifest V3 扩展。开启后，它会在 Netflix 首页、电影、节目、最新、分类、我的片单和搜索页自动隐藏不符合字幕条件的影片卡片；播放页仍可加载自己的字幕文件。
...
2. 在 **Assets** 中下载 `netflix-home-subtitle-filter-v1.0.0.zip`，然后解压。
